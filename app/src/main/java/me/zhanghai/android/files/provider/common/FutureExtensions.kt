...
fun <T> Deferred<T>.asFuture(): Future<T> = object : Future<T> {
    ...
    private fun getCompleted(): T = try {
...
fun <T> Promise<T, *>.asFuture(): Future<T> = object : Future<T> {
    ...
    @Throws(ExecutionException::class, InterruptedException::class, TimeoutException::class)
    override fun get(timeout: Long, unit: TimeUnit?): T = tryRetrieve { retrieve(timeout, unit) }

    @Throws(ExecutionException::class, InterruptedException::class, TimeoutException::class)
    private inline fun tryRetrieve(retrieve: () -> T): T = try {
        retrieve()
    } catch (e: Exception) {
